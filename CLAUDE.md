...
- **`cmd/mcp.go`** — MCP server using `mark3labs/mcp-go`. Registers 6 tools that reuse the same `client.Client`. Tool errors return `mcp.NewToolResultError()` (tool-level), not Go errors (transport-level).
